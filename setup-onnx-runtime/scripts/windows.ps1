...
$DestDir = $args[1]
if ([string]::IsNullOrWhiteSpace($DestDir)) { throw "Usage: windows.ps1 <ortVersion> <destDir> [archId] [strategy]" }

$ArchId = if ($args.Count -ge 3 -and -not [string]::IsNullOrWhiteSpace($args[2])) { $args[2] } else { "" }
