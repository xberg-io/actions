...
- **gnu-zigbuild build actions: add a `linux-features` input so OpenSSL-dependent crates can vendor OpenSSL on the cargo-zigbuild path.** With the glibc-2.28 floor active, the gnu Linux artifacts (CLI, Go/Java/C#/Dart/C FFI, Elixir NIF) compile `openssl-sys` under `zigcc`, which is invoked with only `-I /usr/include` and so misses the Debian/Ubuntu multiarch header dir (`/usr/include/<triple>/openssl/opensslconf.h`) — failing with `fatal error: 'openssl/opensslconf.h' file not found`. This is the same class of failure already fixed for `build-swift-artifactbundle` (which builds the two linux-gnu slices on macOS). Generalize that fix: each affected build action gains a `linux-features` input (`LINUX_FEATURES` env / `INPUT_LINUX_FEATURES` in the script-based ones) whose features are appended via `--features` **only on the cargo-zigbuild path** (target is `*-linux-gnu` AND `glibc-version` is set). kreuzberg's `publish.yaml` passes `kreuzberg/openssl-vendored`, flipping the unified `openssl-sys` to its `vendored` feature so OpenSSL is built from source for the target and no system headers are needed — also yielding a self-contained binary with no runtime `libssl` dependency (correct for shipped artifacts). The input is empty by default and ignored on musl/macOS/Windows builds, so other polyrepo consumers are unaffected. Validated locally in an `ubuntu:22.04` container: the system-OpenSSL zigbuild reproduces `opensslconf.h` not found, and `native-tls/vendored` compiles `openssl-sys` to completion under `cargo zigbuild --target x86_64-unknown-linux-gnu.2.28`. (`build-rust-cli/action.yml`, `build-go-ffi/{action.yml,scripts/build.py}`, `build-rust-ffi/{action.yml,scripts/build.py}`, `build-java-natives/{action.yml,scripts/build.py,scripts/musl_builder.py}`, `build-csharp-natives/{action.yml,scripts/build.py,scripts/musl_builder.py}`, `build-elixir-natives/{action.yml,scripts/build.py,scripts/musl_builder.py}`, `tests/test_build_ffi.py`, and kreuzberg `.github/workflows/publish.yaml`)
- **`build-swift-artifactbundle`: fix Linux cross-compilation with OpenSSL vendoring.** The build action compiled Linux slices (aarch64 and x86_64) using `cargo zigbuild` on macOS, but `ureq` with `native-tls` pulled `openssl-sys` → `native-tls/vendored`, which failed cross-compilation because the host's C compiler / Zig linker lacked an OpenSSL source build path. Updated both Linux zigbuild commands to pass `--features kreuzberg/openssl-vendored`, enabling the unified native-tls dep to use its `vendored` feature. OpenSSL is now built from source for the target architecture using Zig as the cross-linker, allowing the bundle to assemble on macOS with all six platform slices (arm64/x86_64 macOS, arm64 iOS + arm64/x86_64 iOS simulator, aarch64/x86_64 Linux). This unblocks the Publish-Release job, which depends on the bundle to finalize GitHub release + publish Elixir Hex and Zig packages. (`build-swift-artifactbundle/scripts/build.sh`)
