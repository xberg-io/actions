...
the version expected by `kreuzberg-dev/pre-commit-hooks` (currently `2.20.0`);
...
```yaml
- uses: kreuzberg-dev/actions/setup-c-cpp-tools@v1
```
...
| ---------------------- | -------- | --------- | ---------------------------------------------------------------------------------------------- |
| `cppcheck-version`     | no       | `2.20.0`  | cppcheck version to install. Must match the pin in `kreuzberg-dev/pre-commit-hooks`.           |
