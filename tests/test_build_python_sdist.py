...
def test_runs_rewrite_then_maturin():
    ...
    assert "maturin sdist" in content
...
def test_supports_both_package_dir_and_manifest_path_modes():
    content = _read()
    # manifest-path mode uses -m; package-dir mode cd's (under workspace) then --out.
    assert 'maturin sdist -m "${INPUT_MANIFEST_PATH}"' in content
    assert 'cd "${GITHUB_WORKSPACE}/${INPUT_PACKAGE_DIR}"' in content
